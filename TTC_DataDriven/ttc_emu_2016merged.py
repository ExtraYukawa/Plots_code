#==============
# Last used:
# python ttc_emu_2016merged.py --era 2016merged --saveDir 2016merged_em   
#==============
...
def histos_book(flist, filters, variables, isData = "False", isFake = "False", era="2016APV"):
  ...
  if not isData:
    df_xyz_tree = df_xyz_tree.Define("trigger_SF","trigger_sf_em_"+era+"(ttc_l1_pt,ttc_l2_pt,ttc_l1_eta,ttc_l2_eta)")
    # check if the events are fake or not
    if isFake:
      df_xyz_tree = df_xyz_tree.Define("fakelep_weight","fakelepweight_em_"+era+"(ttc_1P1F,ttc_0P2F,ttc_lep1_faketag,muon_conePt[ttc_l1_id],ttc_l1_eta,electron_conePt[ttc_l2_id],ttc_l2_eta, "+str(isData).lower()+")")
  ...
  if not isData:
    ...
    if "doubleeg" in str(flist[0]).split('/')[-1].lower():
      ...
      print ("singleEle")
      df_xyz_trigger = for_singleele_trigger_emuchannel(df_xyz, era)
    elif "singlemu" in str(flist[0]).split('/')[-1].lower():
      print ("singleMu")
      df_xyz_trigger = for_singlemuon_trigger_emuchannel(df_xyz, era)
    elif "muoneg" in str(flist[0]).split('/')[-1].lower():
      print ("muonEG")
      df_xyz_trigger = for_cross_trigger(df_xyz, era)
    else:
...
if opts.era == "2016APV":
  ...
  muonEle_names   = get_filelist(path, ["MuonEG_B2.root","MuonEG_C.root","MuonEG_D.root","MuonEG_E.root","MuonEG_F.root"])
elif opts.era == "2016postAPV":
  ...
  muonEle_names   = get_filelist(path, ["MuonEG_F.root","MuonEG_G.root","MuonEG_H.root"])
elif opts.era == "2016merged":
  ...
  muonEle_names_APV       = get_filelist(pathAPV, ["MuonEG_B2.root","MuonEG_C.root","MuonEG_D.root","MuonEG_E.root","MuonEG_F.root"])
  muonEle_names_postAPV   = get_filelist(pathpostAPV, ["MuonEG_F.root","MuonEG_G.root","MuonEG_H.root"])
else:
...
def TTC_Analysis(opts):
  ...
  # import selections
  filters_mc, filters_mc_fake, filters_data, filters_data_fake = selections("emu")
  ...
  ## MuonEle samples
  ##############
  df_MuonEle_histos_APV = histos_book(muonEle_names_APV, filters_data, variables, True, False, "2016APV") #isData, isFake 
  ...
  for ij in range(0,len(variables)):
    ...
    histos.append(h_MuonEle_APV.Clone())
