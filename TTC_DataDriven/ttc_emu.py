...
def histos_book(flist, filters, hists_name, histos_bins, histos_bins_low, histos_bins_high, isData = "False", isFake = "False"):
  ...
  if not isData:
    df_xyz_tree = df_xyz_tree.Define("trigger_SF","trigger_sf_em(ttc_l1_pt,ttc_l2_pt,ttc_l1_eta,ttc_l2_eta)")
    # check if the events are fake or not
    if isFake:
      df_xyz_tree = df_xyz_tree.Define("fakelep_weight","fakelepweight_em_mc(ttc_1P1F,ttc_0P2F,ttc_lep1_faketag,muon_conePt[ttc_l1_id],ttc_l1_eta,electron_conePt[ttc_l2_id],ttc_l2_eta)")
    ...
    if isFake:
      df_xyz_tree = df_xyz_tree.Define("fakelep_weight","fakelepweight_em_data(ttc_1P1F,ttc_0P2F,ttc_lep1_faketag,muon_conePt[ttc_l1_id],ttc_l1_eta,electron_conePt[ttc_l2_id],ttc_l2_eta)")
...
def TTC_Analysis(opts):
  ...
  ## SingleEle samples
  ##############
  df_MuonEle_histos = histos_book(muonEle_names, filters_data, hists_name, histos_bins, histos_bins_low, histos_bins_high, True, False) #isData, isFake 
  df_FakeLep_MuonEle_histos = histos_book(muonEle_names, filters_data_fake, hists_name, histos_bins, histos_bins_low, histos_bins_high, True, True) #isData, isFake 
...
#  ROOT.RDF.RunGraphs({df_ZZG_histo, df_ZZ_histo, df_ggZZ_4e_histo,df_ggZZ_4mu_histo, df_ggZZ_4tau_histo, df_ggZZ_2e2mu_histo,df_ggZZ_2e2tau_histo, df_ggZZ_2mu2tau_histo, df_TTZ_histo,df_TTG_histo, df_WWZ_histo, df_WZG_histo,df_WZZ_histo, df_ZZZ_histo, df_WZTo3L_histo,df_WZTo2L_histo, df_ZG_histo})
    ...
    c1 = plot.draw_plots(opts, histos, 1, hists_name[ij], 1)
    del histos[:]
