...
def histos_book(flist, filters, hists_name, histos_bins, histos_bins_low, histos_bins_high, isData = "False", isFake = "False"):
  ...
  if not isData:
    ...
    if isFake:
      df_xyz_tree = df_xyz_tree.Define("fakelep_weight","fakelepweight_mm_mc(ttc_1P1F,ttc_0P2F,ttc_lep1_faketag,muon_conePt[ttc_l1_id],ttc_l1_eta,muon_conePt[ttc_l2_id],ttc_l2_eta)")
    ...
    if isFake:
      df_xyz_tree = df_xyz_tree.Define("fakelep_weight","fakelepweight_mm_data(ttc_1P1F,ttc_0P2F,ttc_lep1_faketag,muon_conePt[ttc_l1_id],ttc_l1_eta,muon_conePt[ttc_l2_id],ttc_l2_eta)")
  ...
  if not isData:
    ...
    if "doublemu" in str(flist[0]).split('/')[-1].lower():
      print ("doubleMu")
      df_xyz_trigger = for_dimuon_trigger(df_xyz, opts.era)
    elif "singlemu" in str(flist[0]).split('/')[-1].lower():
      print ("singleMu")
      df_xyz_trigger = for_singlemuon_trigger_mumuchannel(df_xyz, opts.era)
    else:
...
def TTC_Analysis(opts):
  ...
  df_DoubleMu_histos = histos_book(doubleMu_names, filters_data, hists_name, histos_bins, histos_bins_low, histos_bins_high, True, False) #isData, isFake 
  df_FakeLep_DoubleMu_histos = histos_book(doubleMu_names, filters_data_fake, hists_name, histos_bins, histos_bins_low, histos_bins_high, True, True) #isData, isFake, what is data fake? 

  df_SingleMu_histos = histos_book(singleMu_names, filters_data, hists_name, histos_bins, histos_bins_low, histos_bins_high, True, False) #isData, isFake
  df_FakeLep_SingleMu_histos = histos_book(singleMu_names, filters_data_fake, hists_name, histos_bins, histos_bins_low, histos_bins_high, True, True) #isData, isFake
...
#  ROOT.RDF.RunGraphs({df_ZZG_histo, df_ZZ_histo, df_ggZZ_4e_histo,df_ggZZ_4mu_histo, df_ggZZ_4tau_histo, df_ggZZ_2e2mu_histo,df_ggZZ_2e2tau_histo, df_ggZZ_2mu2tau_histo, df_TTZ_histo,df_TTG_histo, df_WWZ_histo, df_WZG_histo,df_WZZ_histo, df_ZZZ_histo, df_WZTo3L_histo,df_WZTo2L_histo, df_ZG_histo})
    ...
    h_DoubleMu = df_DoubleMu_histos[ij].Clone()
    h_SingleMu = df_SingleMu_histos[ij].Clone()
    h_fakelep_DoubleMu = df_FakeLep_DoubleMu_histos[ij].Clone()
