#==============
# Last used:
# python ttc_ee_2016merged.py --era 2016merged --saveDir 2016merged_ee
#==============
...
def histos_book(flist, filters, variables, isData = "False", isFake = "False", era="2016APV"):
  ...
  if not isData:
    ...
    if isFake:
      df_xyz_tree = df_xyz_tree.Define("fakelep_weight","fakelepweight_ee_"+era+"(ttc_1P1F,ttc_0P2F,ttc_lep1_faketag,electron_conePt[ttc_l1_id],ttc_l1_eta,electron_conePt[ttc_l2_id],ttc_l2_eta, "+str(isData).lower()+")")
  ...
  if not isData:
    ...
    if "doubleeg" in str(flist[0]).split('/')[-1].lower():
      print ("doubleEle")
      df_xyz_trigger = for_diele_trigger(df_xyz, era)
    elif "singleeg" in str(flist[0]).split('/')[-1].lower():
      print ("singleEle")
      df_xyz_trigger = for_singleele_trigger_eechannel(df_xyz, era)
    else:
...
if opts.era == "2016APV":
  ...
  doubleEle_names_APV     = get_filelist(pathAPV, ["DoubleEG_B2.root","DoubleEG_C.root","DoubleEG_D.root","DoubleEG_E.root","DoubleEG_F.root"])
  singleEle_names_APV     = get_filelist(pathAPV, ["SingleEG_B2.root","SingleEG_C.root","SingleEG_D.root","SingleEG_E.root","SingleEG_F.root"])
  doubleEle_names_postAPV = get_filelist(pathpostAPV, ["DoubleEG_F.root","DoubleEG_G.root","DoubleEG_H.root"])
  singleEle_names_postAPV = get_filelist(pathpostAPV, ["SingleEG_F.root","SingleEG_G.root","SingleEG_H.root"])
else:
  raise Exception ("select correct subEra!")
...
def TTC_Analysis(opts):
  ...
  ## DoubleEle samples
  ##############
  df_DoubleEle_histos_APV = histos_book(doubleEle_names_APV, filters_data, variables, True, False, "2016APV") #isData, isFake
  df_DoubleEle_histos_postAPV = histos_book(doubleEle_names_postAPV, filters_data, variables, True, False, "2016postAPV") #isData, isFake
  df_FakeLep_DoubleEle_histos_APV = histos_book(doubleEle_names_APV, filters_data_fake, variables, True, True, "2016APV") #isData, isFake, what is data fake?
  df_FakeLep_DoubleEle_histos_postAPV = histos_book(doubleEle_names_postAPV, filters_data_fake, variables, True, True, "2016postAPV") #isData, isFake, what is data fake?
  print ("DoubleEle both genuine and fake histo loading complete!")
  
  ##############
  ## SingleEle samples
  ##############
  df_SingleEle_histos_APV = histos_book(singleEle_names_APV, filters_data, variables, True, False, "2016APV") #isData, isFake
  df_SingleEle_histos_postAPV = histos_book(singleEle_names_postAPV, filters_data, variables, True, False, "2016postAPV") #isData, isFake
  df_FakeLep_SingleEle_histos_APV = histos_book(singleEle_names_APV, filters_data_fake, variables, True, True, "2016APV") #isData, isFake
  df_FakeLep_SingleEle_histos_postAPV = histos_book(singleEle_names_postAPV, filters_data_fake, variables, True, True, "2016postAPV") #isData, isFake
  ...
  for ij in range(0,len(variables)):
    ...
    h_DoubleEle_APV = df_DoubleEle_histos_APV[ij].Clone()
    ...
    h_DoubleEle_postAPV = df_DoubleEle_histos_postAPV[ij].Clone()
    ...
    # Fake
    h_fakelep_DoubleEle_APV = df_FakeLep_DoubleEle_histos_APV[ij].Clone()
    ...
    h_fakelep_DoubleEle_postAPV = df_FakeLep_DoubleEle_histos_postAPV[ij].Clone()
    ...
    # h_DY = add_two_histos_new(h_DY_APV, h_DY_postAPV) #not sure what is problem
    ...
    h_fakelep_DoubleEle_APV.Add(h_fakelep_DoubleEle_postAPV)
    histos.append(h_fakelep_DoubleEle_APV.Clone())

    h_fakelep_SingleEle_APV.Add(h_fakelep_SingleEle_postAPV)
    histos.append(h_fakelep_SingleEle_APV.Clone())
    h_DoubleEle_APV.Add(h_DoubleEle_postAPV)
    histos.append(h_DoubleEle_APV.Clone()) 
    h_SingleEle_APV.Add(h_SingleEle_postAPV)
