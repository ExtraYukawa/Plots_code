#==============
# Last used:
# python DY_emu_2016.py --subEra APV
# python DY_emu_2016.py --subEra postAPV
#==============
...
def TTC_Analysis(opts):
  ...
  #filters="OPS_region==2 && OPS_2P0F && OPS_z_mass>60 && OPS_z_mass<120 && OPS_l1_pt>30 && OPS_l2_pt>20 && OPS_drll>0.3 && Flag_goodVertices && Flag_globalSuperTightHalo2016Filter && Flag_HBHENoiseFilter && Flag_HBHENoiseIsoFilter && Flag_EcalDeadCellTriggerPrimitiveFilter && Flag_BadPFMuonFilter && Flag_eeBadScFilter && Flag_ecalBadCalibFilter && nHad_tau==0" # probably wrong
  ...
  print ("MuonEle Files\n")
  print(' '.join(map(str, muonEle_names)))
...
#  ROOT.RDF.RunGraphs({df_ZZG_histo, df_ZZ_histo, df_ggZZ_4e_histo,df_ggZZ_4mu_histo, df_ggZZ_4tau_histo, df_ggZZ_2e2mu_histo,df_ggZZ_2e2tau_histo, df_ggZZ_2mu2tau_histo, df_TTZ_histo,df_TTG_histo, df_WWZ_histo, df_WZG_histo,df_WZZ_histo, df_ZZZ_histo, df_WZTo3L_histo,df_WZTo2L_histo, df_ZG_histo})
    ...
    histos.append(h_SingleEle.Clone()) #9
    histos.append(h_SingleMu.Clone()) #10
    histos.append(h_MuonEle.Clone()) #11

    for i in range(0,12): # fixme-gkole
      histos[i]=overunder_flowbin(histos[i])

    c1 = plot_DYregion.draw_plots(opts, histos, 1, hists_name[ij], 1, False)
    del histos[:]
