...
def draw_plots(hist_array=[], draw_data=0, x_name='', isem=0):
    ...
    for i in range(1, Nbins+1):
        ...
        if max_yields_temp > max_yields:
            max_yields = max_yields_temp

    max_yields_data = 0
    for i in range(1, Nbins+1):
        max_yields_data_temp = Data.GetBinContent(i)
        if max_yields_data_temp > max_yields_data:
            max_yields_data = max_yields_data_temp
    ...
    gr = ROOT.TGraphAsymmErrors(len(x), np.array(x), np.array(y), np.array(
        xerror_l), np.array(xerror_r), np.array(yerror_d), np.array(yerror_u))
    ...
    gr.Draw("SAME 2")
    if 'OPS_l1_pt' in x_name:
        set_axis(h_stack, 'x', 'pt of leading lepton', True)
    if 'OPS_l1_eta' in x_name:
        set_axis(h_stack, 'x', '#eta of leading lepton', False)
    if 'OPS_l1_phi' in x_name:
        set_axis(h_stack, 'x', 'phi of leading lepton', False)
    if 'OPS_l2_pt' in x_name:
        set_axis(h_stack, 'x', 'pt of subleading lepton', True)
    if 'OPS_l2_eta' in x_name:
        set_axis(h_stack, 'x', '#eta of subleading lepton', False)
    if 'OPS_l2_phi' in x_name:
        set_axis(h_stack, 'x', 'phi of subleading lepton', False)
    if 'OPS_z_mass' in x_name:
        set_axis(h_stack, 'x', 'Z mass', True)
    if 'OPS_z_pt' in x_name:
        set_axis(h_stack, 'x', 'Z pt', True)
    if 'OPS_z_eta' in x_name:
        set_axis(h_stack, 'x', 'Z #eta', False)
    if 'OPS_z_phi' in x_name:
        set_axis(h_stack, 'x', 'Z phi', False)
    # WZ region
    if 'wl_pt' in x_name:
        set_axis(h_stack, 'x', 'W lepton pt', True)
    if 'wl_eta' in x_name:
        set_axis(h_stack, 'x', 'W lepton #eta', False)
    if 'zl1_pt' in x_name:
        set_axis(h_stack, 'x', 'Z Leading lepton pt', True)
    if 'zl1_eta' in x_name:
        set_axis(h_stack, 'x', 'Z Leading lepton #eta', False)
    if 'zl2_pt' in x_name:
        set_axis(h_stack, 'x', 'Z Subleading lepton pt', True)
    if 'zl2_eta' in x_name:
        set_axis(h_stack, 'x', 'Z Subleading lepton #eta', False)
    if 'met' in x_name:
        set_axis(h_stack, 'x', 'MET', True)
    if 'zmass' in x_name:
        set_axis(h_stack, 'x', 'Z mass', True)
