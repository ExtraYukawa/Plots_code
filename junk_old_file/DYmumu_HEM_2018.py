#==============
# Last used: python3 DYmumu_HEM_2018.py --period ABvsCD --saveDir /eos/user/g/gkole/www/public/TTC/20Apr2022/DYmumu1Jets
#==============
...
doubleMu_namesAB = ROOT.std.vector('string')()
doubleMu_namesCD = ROOT.std.vector('string')()

#for f in ["EGammaA.root","EGammaB.root","EGammaC.root","EGammaD_0.root","EGammaD_1.root"]:
if opts.period == "B":
  for f in ["DoubleMuonB.root"]:
    singleEle_names.push_back(path+f)
elif opts.period == "C":
  for f in ["DoubleMuonC.root"]:
    singleEle_names.push_back(path+f)
elif opts.period == "D":
  for f in ["DoubleMuonD_0.root", "DoubleMuonD_1.root"]:
    singleEle_names.push_back(path+f)
elif opts.period == "ABvsCD":
  for f in ["DoubleMuonA.root", "DoubleMuonB.root"]:
    doubleMu_namesAB.push_back(path+f)
  for f in ["DoubleMuonC.root", "DoubleMuonD_0.root", "DoubleMuonD_1.root"]:
    doubleMu_namesCD.push_back(path+f)
else:
...
def HEM_Eta_Vs_Phi(opts):
    ...
    DYfilters="OPS_region==1 && OPS_2P0F && OPS_z_mass>60 && OPS_z_mass<120 && OPS_l1_pt>30 && OPS_l2_pt>20 && OPS_drll>0.3 && Flag_goodVertices && Flag_globalSuperTightHalo2016Filter && Flag_HBHENoiseFilter && Flag_HBHENoiseIsoFilter && Flag_EcalDeadCellTriggerPrimitiveFilter && Flag_BadPFMuonFilter && Flag_eeBadScFilter && Flag_ecalBadCalibFilter && nHad_tau==0"    
    ...
    c1.SaveAs('DYmumu_HEM_'+opts.period+'.pdf')
...
def PreHEM_PostHEM(opts):
  ...
  print ("File opens")
  df_doubleMu_treeAB = ROOT.RDataFrame("Events", doubleMu_namesAB)
  print(' '.join(map(str, doubleMu_namesAB)))
  
  df_doubleMu_treeCD = ROOT.RDataFrame("Events", doubleMu_namesCD)
  print(' '.join(map(str, doubleMu_namesCD)))

  df_doubleMu_treeAB = df_doubleMu_treeAB.Define("jet1_pt","Jet_pt[0]")
  df_doubleMu_treeAB = df_doubleMu_treeAB.Define("jet1_eta","Jet_eta[0]")
  df_doubleMu_treeAB = df_doubleMu_treeAB.Define("jet1_phi","Jet_phi[0]")
  df_doubleMu_treeAB = df_doubleMu_treeAB.Define("jet2_pt","Jet_pt[1]")
  df_doubleMu_treeAB = df_doubleMu_treeAB.Define("jet2_eta","Jet_eta[1]")
  df_doubleMu_treeAB = df_doubleMu_treeAB.Define("jet2_phi","Jet_phi[1]")
  
  df_doubleMu_treeCD = df_doubleMu_treeCD.Define("jet1_pt","Jet_pt[0]")
  df_doubleMu_treeCD = df_doubleMu_treeCD.Define("jet1_eta","Jet_eta[0]")
  df_doubleMu_treeCD = df_doubleMu_treeCD.Define("jet1_phi","Jet_phi[0]")
  df_doubleMu_treeCD = df_doubleMu_treeCD.Define("jet2_pt","Jet_pt[1]")
  df_doubleMu_treeCD = df_doubleMu_treeCD.Define("jet2_eta","Jet_eta[1]")
  df_doubleMu_treeCD = df_doubleMu_treeCD.Define("jet2_phi","Jet_phi[1]")

  print ("Filter applied \n")
  print (DYfilters)
  
  df_doubleMuAB = df_doubleMu_treeAB.Filter(DYfilters)
  df_doubleMuCD = df_doubleMu_treeCD.Filter(DYfilters)
  
  df_doubleMuABpreHEM = df_doubleMuAB.Filter("run < 319077")
  
  print ("\ntrigger")
  df_doubleMuABpreHEM_trigger  = for_dimuon_trigger(df_doubleMuABpreHEM)
  df_doubleMuCDpostHEM_trigger  = for_dimuon_trigger(df_doubleMuCD)
  
  df_doubleMu_preHEM_histos = []
  df_doubleMu_postHEM_histos = []

  for hist in hists_name:
    df_doubleMu_preHEM_histo  = df_doubleMuABpreHEM_trigger.Histo1D((hist,"",histos_bins[hist] ,histos_bins_low[hist], histos_bins_high[hist]), hist)  
    df_doubleMu_preHEM_histos.append(df_doubleMu_preHEM_histo)
    
    df_doubleMu_postHEM_histo = df_doubleMuCDpostHEM_trigger.Histo1D((hist,"",histos_bins[hist] ,histos_bins_low[hist], histos_bins_high[hist]), hist)
    df_doubleMu_postHEM_histos.append(df_doubleMu_postHEM_histo)

  # Request a cut-flow report (also does not run the computation yet!)
  report_preHEM = df_doubleMuABpreHEM_trigger.Report()
  ...
  for ij in range(0,len(hists_name)):
    h_SingleEle_preHEM  = df_doubleMu_preHEM_histos[ij].GetValue()
    # print ("Integral for preHEM",  h_SingleEle_preHEM.Integral())
    histospreHEM.append(h_SingleEle_preHEM)
    h_SingleEle_postHEM  = df_doubleMu_postHEM_histos[ij].GetValue()
  ...
  for ij in range(0, len(histospreHEM)):
    ...
    c1.SaveAs(opts.saveDir+'/DYmumu_preVSpostHEM'+histospreHEM[ij].GetName()+'_'+opts.period+'.pdf')
...
def HEM_1D_plots(opts):
  ...
  for ij in range(0, len(histos)):
    ...
    c1.SaveAs('/eos/user/g/gkole/www/public/TTC/19Apr2022/DY_'+histos[ij].GetName()+'_'+opts.period+'.pdf')
    del c1
  # c1.SaveAs('TTbar_HEM_'+opts.period+'.pdf')
  print ("save the canvas")
