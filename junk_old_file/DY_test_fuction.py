...
def MC_histos_book(flist, filters, hists_name, histos_bins, histos_bins_low, histos_bins_high):
  ...
  df_xyz_tree = df_xyz_tree.Define("trigger_SF","trigger_sf_mm(OPS_l1_pt,OPS_l2_pt,OPS_l1_eta,OPS_l2_eta)")
...
def TTC_Analysis(opts):
  ...
  histooo = []
  print type(df_DY_histos)
  for ij in range(0,len(hists_name)):
    # for h in range(0, len(df_DY_histos)):
    print "ij", ij
    print type(df_DY_histos[ij])
    # print "integral: ", df_DY_histos[ij].Integral()
    
    #print "append: ", histooo.append(df_DY_histos[ij].Clone())
  sys.exit(1)
  #h = df_DY_histos[0].GetValue()
  #print "integral h[0]", h.Integral()
...
#    h_QCD300toinf.Scale(QCD300toinf_xs/QCD300toinf_ev)
    ...
    histos.append(h_DoubleMu.Clone())  #9 
